...
void AddPipe(map<int, Pipe>& pipes, int& idp)
{
	...
	cout << "Enter the pipename" << endl;

	getline(cin, pipes[idp].namepipe);

	cout << "Enter the length of pipe" << endl;

	pipes[idp].length = Input<float>(2, 1, 0, 1);

	cout << "Enter the diametr of pipe" << endl;
...
void AddNPZ(map<int, NPZ>& CSs, int& idc)
{
	...
	cout << "Enter the name of npz" << endl;

	getline(cin, CSs[idc].namenpz);

	cout << "Enter the count of factories" << endl;

	CSs[idc].countfactry = Input<int>(2, 1, 0, 1);

	cout << "Enter the count of working factories" << endl;
...
void RedactNPZ(map<int, NPZ>& CSs, int& idc)
{
	...
	if (CSs[idc].countfactry > 0)
	{
		...
		cout << "Enter the count factory" << endl;
...
void Save(map<int, Pipe>& pipes, int& idp, std::map<int, NPZ>& CSs, int& idc)
{
	...
	if (pipes.size() > 0) {
		...
		cout << "pipes were saved succesfully" << endl << endl;
	}
	else {
		cout << "couldn't save because theres nothing to save" << endl << endl;
	}
...
void Find(map<int, Pipe>& pipes, std::map<int, NPZ>& CSs, int idp, int idc) {
	...
	cout << "By which criteria should the program search for an object (if pipe: 1 - name, 2 - repair; if CS: 1 - name, 2 - by the percentage of unused workshops)" << endl;
